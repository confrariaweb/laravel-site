...
@section('title', __('Tipode de páginas'))
...
<div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
        <div class="x_title">
            <h2>
                {{ __('Lista de tipos de paginas') }} 
                <small>{{ _('Lista de tipos de paginas') }}</small>
            </h2>
            @include('dashboard.gentelella.page-types.partials.panel_toolbox')
            <div class="clearfix"></div>
        </div>
        <div class="x_content">
            <p class="text-muted font-13 m-b-30">
                {{ __('Abaixo segue a sua lista de tipos de paginas') }}
            </p>
            <table id="datatable-pages" class="table table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $page_type->title }}</td>
                        <td>{{ $page_type->slug }}</td>
                        <td>{!! status($page_type->status) !!}</td>
                        <td class="text-right">
                            <a href="{{ route('admin.page-types.edit', $page_type->id) }}" class="btn btn-success btn-xs">
                                <i class="fa fa-edit"></i> {{ __('Editar') }}  
                            </a>                           
                            <a href="#" class="btn btn-danger btn-xs" onclick="if(confirm('Tem certeza que deseja deletar?')){ document.getElementById('destroy_{{ $page_type->id }}').submit(); }">
                                <i class="fa fa-trash-o"></i> {{ __('Deletar') }}  
                            </a>        
                            {!! Form::open([ 'method'  => 'DELETE', 'route' => ['admin.page-types.destroy', $page_type->id], 'id' => 'destroy_' . $page_type->id ]) !!}
                                {!! Form::hidden('id', $page_type->id) !!}
                            {!! Form::close() !!}
                        </td>
                    </tr>
                    @endforeach
