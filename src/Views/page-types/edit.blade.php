...
@section('title', __('Tipo de páginas'))
...
<div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
            <div class="x_title">
                <h2>
                    {{ __('Editar o tipo de página ' . $page_type->title) }} 
                    <small>{{ __('Ao criar um tipo pagina ele sera vinculado a conta.') }}</small>
                </h2>
                @include('dashboard.gentelella.page-types.partials.panel_toolbox')
                <div class="clearfix"></div>
            </div>
            <div class="x_content">
                {!! Form::model($page_type, ['route' => ['admin.page-types.update', $page_type->id], 'files' => true, 'method' => 'put', 'class' => 'form-horizontal form-label-left']) !!}
                    @include('dashboard.gentelella.page-types.partials.form')
                {!! Form::close() !!}
            </div>
        </div>
    </div>
